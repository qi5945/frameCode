...
object XxPreferenceManager {
    ...
    inline fun <reified T> getValue(sharedPreferences: SharedPreferences, key: String, defValue: T): T? {
        if (defValue is String) {
            ...
            val string = sharedPreferences.getString(key, Gson().toJson(defValue))
            return Gson().fromJson(string, T::class.java) as T
        } else if (defValue is List<*>) {
            val string = sharedPreferences.getString(key, Gson().toJson(defValue))
            return return Gson().fromJson(string, object : TypeToken<List<*>>() {
            }.type) as T
        }
        return null
    }
